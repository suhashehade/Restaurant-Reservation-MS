...
It looks good, but the most interesting things are Sort(Cost 9%) in ```GROUP BY```, and Hash Match (Cost 19% and 16%) in ```INNER JOIN```, that becuase of FKs are not indexes.
